...
import br.com.impacta.meucondominio.infrastructure.entity.SequenceUsuario;
...
public class GeradorSequenceServiceImpl implements GeradorSequenceService {
    ...
    public long gerarSequenceUsuario(String nomeSequence) {
        SequenceUsuario counter = mongoOperations.findAndModify(query(where("_id").is(nomeSequence)),
                new Update().inc("seq",1), options().returnNew(true).upsert(true),
                SequenceUsuario.class);
        return !Objects.isNull(counter) ? counter.getSeq() : 1;
    }
}
